...
public:
   ...
   void basic_setup() {
      ...
      create_accounts({ N(eosio.token), N(eosio.stake), N(eosio.saving), 
                        ...
                        N(eosio.ppay), N(eosio.rex), N(resource) });
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("eosio")  + get_balance("eosio.ramfee") + get_balance("eosio.stake") );
   }
   ...
   worbli_system_tester(Lambda setup) {
      setup(*this);
   ...
   action_result push_resource_action( const account_name& signer, const action_name &name, const variant_object &data, bool auth = true ) {
         string action_type_name = resource_abi_ser.get_action_type(name);

         action act;
         act.account = N(resource);
         act.name = name;
         act.data = resource_abi_ser.variant_to_binary( action_type_name, data, abi_serializer_max_time );
   ...
   action_result setwgstate( string last_inflation_print, string last_metric_read ) {
      return push_action( config::system_account_name, N(setwgstate), mvo()
   ...
   action_result addupdsource( account_name account, uint8_t in_out ) {
      return push_resource_action( N(resource), N(addupdsource), mvo()
           ( "account", account )
           ( "in_out", in_out )
      );
   }
   ...
   action_result updconfig( bool paused, uint32_t emadraglimit ) {
      return push_resource_action( N(resource), N(updconfig), mvo()
           ( "paused", paused )
   ...
   action_result init( string start ) {
      return push_resource_action( N(resource), N(init), mvo()
           ( "start", start )
      );
   }

   action_result settotal(name source, float total_cpu_quantity, float total_net_quantity, string timestamp) {
      return push_resource_action( N(worbli.admin), N(settotal), mvo()
           ( "source", source )
   ...
   fc::variant get_inflation(uint64_t timestamp) {
      ...
      return data.empty() ? fc::variant() : resource_abi_ser.binary_to_variant( "daily_inflation", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_metrics(uint64_t timestamp) {
      vector<char> data = get_row_by_account( N(eosio), N(eosio), N(metrics), timestamp );
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "metric", data, abi_serializer_max_time );
   }

   fc::variant get_wglobal_state() {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, N(wglobal), N(wglobal) );
      if (data.empty()) std::cout << "\nData is empty\n" << std::endl;
